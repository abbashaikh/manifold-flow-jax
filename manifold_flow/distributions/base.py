...
class Distribution(eqx.Module):
    ...
    def log_prob(self, inputs, context=None):
        ...
        Args:
            inputs: pytree input variables.
            context: pytree or None, conditioning variables. If a pytree, it must have the same
                number or rows as the inputs. If None, the context is ignored.

        Returns:
            A pytree of shape [input_size], the log probability of the inputs given the context.
        """
    ...
    def sample(self, num_samples, context=None, batch_size=None):
        ...
        Args:
            ...
            context: pytree or None, conditioning variables. If None, the context is ignored.
            batch_size: int or None, number of samples per batch. If None, all samples are generated
                in one batch.

        Returns:
            A pytree containing the samples, with shape [num_samples, ...] if context is None, or
        ...
        if batch_size is None:
            ...
            return jax.lax.concatenate(samples, dimension=0)
    ...
    def sample_and_log_prob(self, num_samples, context=None):
        ...
        Returns:
            A tuple of:
                * A pytree containing the samples, with shape [num_samples, ...] if context is None,
                  or [context_size, num_samples, ...] if context is given.
                * A pytree containing the log probabilities of the samples, with shape
