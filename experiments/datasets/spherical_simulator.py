...
class SphericalGaussianSimulator(BaseSimulator):
    ...
    def _log_density(self, z_phi, z_eps):
        r = 1.0 + z_eps[0]
        phases_ = self._phases
        cov_ = jnp.diag(self._widths)

        ''' add shifted pdfs of the latent variable z_phi '''
        p_sub = norm.pdf(z_phi, mean=phases_, cov=cov_).reshape(self._latent_dim,)
        ...
        logp_eps = jnp.log(norm.pdf(z_eps, mean=0.0, cov=self._epsilon))
